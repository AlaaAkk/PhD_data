...
if (VBM !=0) and (Eg !=0):
  ...
  plt.axvline(x=VBM-fermi, color='b', linestyle='--')
 # plt.axvline(x=Eg, color='m', linestyle='--')
  plt.axvline(x=fermi-fermi, color='k', linestyle='--')
  plt.axvline(x=CBM-fermi, color='r', linestyle='--')
    ...
    for s in species:
        ...
        for ss in spinstrs:
            if PLOT_DOS_SPECIES:
                ...
                f = open(s+"_l_proj_dos_tetrahedron"+ss+".dat")
            f.readline()
            f.readline()
    ...
    if PLOT_DOS_REVERSED:
       # ax_dos.axhline(0,color='k',ls='--')
        ...
        if PLOT_DOS or PLOT_DOS_TETRAHEDRON:
            # AA
            for ispin in range(max_spin_channel):
                ax_dos.plot(dos[:,ispin]*spinsgn[ispin],energy,color='kk'[ispin])
      ...
      #  ax_dos.axvline(0,color='k',ls='--')
        ...
        if PLOT_DOS_SPECIES or PLOT_DOS_SPECIES_TETRAHEDRON:
            for sp in range(len(species)):
                #AA
                for ispin in range(max_spin_channel):
                    ax_dos.plot(energy,tdos[sp][:,ispin]*spinsgn[ispin],color='brmy'[ispin],linestyle='-',label='%s %s'%(species[sp],['up','down'][ispin]))
                    for l in range(ldos[sp].shape[2]):
        ...
        if PLOT_DOS or PLOT_DOS_TETRAHEDRON:
            for ispin in range(max_spin_channel):
                # AA
                ax_dos.plot(energy,dos[:,ispin]*spinsgn[ispin],color='kk'[ispin])
