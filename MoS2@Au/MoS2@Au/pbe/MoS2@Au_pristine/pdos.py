...
if (VBM !=0) and (Eg !=0):
  ...
  plt.axvline(x=VBM-VBM, color='r', linestyle='--')
 # plt.axvline(x=Eg, color='m', linestyle='--')
#  plt.axvline(x=fermi-VBM, color='k', linestyle='--')
  plt.axvline(x=CBM-VBM, color='g', linestyle='--')
workdir = os.getcwd()
...
plt.plot(elem_df["Energy"]-VBM+0.0,
              elem_df["Mo_up"]+elem_df["S_up"], label="MoS2@Au up", linewidth=1)
plt.plot(elem_df["Energy"]-VBM+0.0,
              -elem_df["Mo_dn"]-elem_df["S_dn"], label="MoS2@Au down", linewidth=1)
elem_df["Energy"] = total_dos_MoS2["Energy"]
elem_df["Dosup"] = total_dos_MoS2["DOS(spin up)"]
elem_df["Dosdown"] = total_dos_MoS2["DOS(spin down)"]
plt.plot(elem_df["Energy"]-VBM,elem_df["Dosup"] , label="MoS2 free up", linewidth=1)
plt.plot(elem_df["Energy"]-VBM,-elem_df["Dosdown"] , label="MoS2 free down", linewidth=1)
#plt.plot(elem_df["Energy"],
#              elem_df["Au_up"], label="Au up", linewidth=3)
#plt.plot(elem_df["Energy"],
#              -elem_df["Au_dn"], label="Au down", linewidth=3)
plt.xlim([-4, 6])
plt.ylim([-500, 500])
