...
total_dos_VS = pd.read_csv(os.path.join(workdir, "KS_DOS_total_raw_tetrahedron_VS.dat"),
                                                skiprows=4,
                                                skipinitialspace = True,
                                                header=None,
                                                sep="\s+",
...
plt.plot(elem_df["Energy"]-VBM,elem_df["Dosup"] , label="MoS2 pristine up", linewidth=1)
plt.plot(elem_df["Energy"]-VBM,-elem_df["Dosdown"] , label="MoS2 pristine down", linewidth=1)
elem_df["Energy"] = total_dos_VS["Energy"]
elem_df["Dosup"] = total_dos_VS["DOS(spin up)"]
elem_df["Dosdown"] = total_dos_VS["DOS(spin down)"]
plt.plot(elem_df["Energy"]-VBM,elem_df["Dosup"] , label="MoS2 VS up", linewidth=1)
plt.plot(elem_df["Energy"]-VBM,-elem_df["Dosdown"] , label="MoS2 VS down", linewidth=1)
