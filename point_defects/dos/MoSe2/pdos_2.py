...
elems = ['Mo','Se']
for elem in elems:
        proj_up = pd.read_csv(os.path.join(workdir, elem+"_l_proj_dos_tetrahedron_spin_up_raw.dat.no_soc"),
                                                      skiprows=4,
                                                      skipinitialspace = True,
                                                      header=None,
                                                      sep="\s+")
        proj_down = pd.read_csv(os.path.join(workdir, elem+"_l_proj_dos_tetrahedron_spin_dn_raw.dat.no_soc"),
                                                        skiprows=4,
                                                        skipinitialspace = True,
                                                        header=None,
                                                        sep="\s+")
...
plt.plot(elem_df["Energy"]-VBM+0.0,
              elem_df["Mo_up"]+elem_df["Se_up"], label="MoSe2 up", linewidth=2)
plt.plot(elem_df["Energy"]-VBM+0.0,
              -elem_df["Mo_dn"]-elem_df["Se_dn"], label="MoSe2 down", linewidth=2)
plt.xlim([-3, 4])
plt.ylim([-300, 300])
