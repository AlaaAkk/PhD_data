...
def moveCOM(height,col,gridinfo):
   ...
   # print('The number of atoms in the molcule  is :\n', n_atoms)
...
# Moving the molecule below the tip
        ...
        n_atoms=108  # I had to choose myself due to constrained atoms
        tp=[scanxyz[col][0],scanxyz[col][1],scanxyz[col][2]]
# Reading the normal modes from get_vibrations.py
        ...
        def read_geo(filename):
            ...
            with open(filename) as f:
                for line in f:
                    ...
                    if t[0] == "#":
                        ...
                        if str(t[4])=='H':
                           elementH += [(str(t[4]))]
                           fdataH += [(float(t[1]), float(t[2]), float(t[3]))]
                        else:
            ...
            fdataH = np.array(fdataH)
            element = np.array(element)
            elementH = np.array(elementH)
            return fdata, element, fdataH, elementH
        def shift_geo(direction,num):
            fdata2=[]
            norm_mode=car_modes(num)
            fdata, element, fdataH, elementH =read_geo('geometry.in')
